...
\begin{landscape}
	\begin{figure}
	\centering
	\caption{Classification of Major Environmental Policies \label{policies}}
	\begin{tikzpicture}[grow = down, scale = 1, semithick, scale = .9]
		\footnotesize
		\tikzstyle{level 1}=[level distance=20mm,sibling distance=9cm]
		\tikzstyle{level 2}=[level distance=5cm,sibling distance=4.5cm]
		\tikzstyle{level 3}=[level distance=4.5cm,sibling distance=4.5cm]
			
		\tikzstyle{s1}=[rectangle, rounded corners, text width=3.5cm, draw, inner sep = 5, fill = white, semithick,fill opacity = 0,text opacity = 1, minimum height = 4cm]
		\tikzstyle{s1-mod}=[rectangle, rounded corners, text width=3.5cm, draw, inner sep = 5, fill = white, semithick,fill opacity = 0,text opacity = 0, draw opacity=0, minimum height = 4cm]
			...
			child{node[s2, text centered]{Command-and-Control Policies}
				...
				child{node[s4]{Standard Setting}
						...
						child{node[s1]{Technology-Based
								\begin{itemize}
									\item Clean electricity portfolio standard
									\item Smokestack scrubber requirements
								\end{itemize}
							}
							edge from parent node[]{}}
					edge from parent node[]{}}
				child{node[s1]{Prohibition/Permission
					\begin{itemize}
	...
	\end{tikzpicture}
	\vspace{.25cm}
	\fignote[1]{Classifications and examples primarily developed from \cite{keohane2016markets}. This chart is not intended to be comprehensive, but illustrates a simple way for classifying common types of environmental policy that makes for a useful reference later in the paper.}
	\end{figure}
\end{landscape}
